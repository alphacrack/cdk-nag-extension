...
### Added
- **Copilot Chat participant `@cdk-nag`** — new `src/chat/participant.ts`. Registered via `vscode.chat.createChatParticipant` and declared in `package.json` under `contributes.chatParticipants`. The handler streams an ask-only guidance block, echoes the user's prompt, and surfaces up to five CDK-NAG diagnostics from the active editor (with rule name lookup from the curated `ruleDocs` map). Natural-language rule explanations and Language Model Tool calls ship in PR 6. Graceful degradation: on hosts without `vscode.chat` (older VS Code, non-Copilot forks), registration returns `undefined` and the rest of the extension continues to work.
