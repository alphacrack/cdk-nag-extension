...
| AI3 | **LM-assisted CodeAction with safety rails** | Feature | L | ✅ Done (PR 7) |
|    | New opt-in setting `cdkNagValidator.enableAiSuggestions` (default `false`) surfaces an **Ask Copilot to suggest a fix** quick-fix only when (a) setting on, (b) `vscode.lm.selectChatModels` available, (c) the rule has no curated static fix in `src/ruleDocs.ts`. Three new source files: `src/ai/scrubber.ts` (8 redaction rules — gitleaks SSN/PAN/phone/IBAN/AWS-creds plus AI-specific AKIA/ASIA/ARN-account/`process.env.*`; pure, no vscode imports), `src/ai/consent.ts` (`ensureAiConsent` modal `showWarningMessage` with Allow once / Always allow / Cancel; globalState-persisted for cross-workspace trust; `resetAiConsent` command), `src/ai/suggestFix.ts` (end-to-end: runtime-gate → consent → ±10 line extract → scrub → `selectChatModels({vendor:'copilot',family:'gpt-4o-mini'})` with fallback → `buildPrompt` with rule-doc context + redaction count header → streamed `sendRequest` inside cancellable `withProgress` → `parseReplacement` fence variants → `WorkspaceEdit` with `WorkspaceEditEntryMetadata.needsConfirmation: true` routing through the Refactor Preview panel). Two new commands: `askCopilotForFix` (internal, `"when": "false"`) and `resetAiConsent` (palette-visible). +56 Jest tests total (22 scrubber, 8 consent, 22 suggestFix, 4 CodeActionProvider AI-branch). Jest grows 135 → 191, full 4-layer suite green. | | | |
